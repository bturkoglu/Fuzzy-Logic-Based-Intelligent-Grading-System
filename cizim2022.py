...
class Cizim(Frame):
    def __init__(self, parent=None):
        ...
        self.y_originler = (190, 410, 630, 850)
        self.x_origin = 100
        self.grafikler = ['Ara Sınav1','Ara Sınav2', 'Genel Sınav', 'Başarı']
        ...
        for grfk in range(4):
            ...
            for i in range(len(self.sira)):
                if bilgi == 'Ara Sınav1':
                    mesaj = self.sira[i]
                    noktalar = self.hesap.ara_sinav1[mesaj]

                elif bilgi == 'Ara Sınav2':
                    mesaj = self.sira[i]
                    noktalar = self.hesap.ara_sinav2[mesaj]
    ...
    def hesapla(self):
        ...
        self.ara_not1    = int(self.ent_ara_not1.get())
        self.ara_not2    = int(self.ent_ara_not2.get())
        self.genel_not  = int(self.ent_genel_not.get())
        self.hesap.hesabaBasla(self.ara_not1, self.ara_not2, self.genel_not)
    ...
    def alanlariYap(self):
        ...
        lab = Label(frmTus, text='Ara Sınav Notu1', width=15)
        ent1 = Entry(frmTus, justify=CENTER, bd=3, width=15)
        ent1.insert(0, self.ara_not1)
        lab.grid(row=row, sticky=W, padx=15)
        ent1.grid(row = row+1, sticky=W, padx=15)
        self.ent_ara_not1 = ent1

        row += 2
        lab = Label(frmTus, text='Ara Sınav Notu2', width=15)
        ent2 = Entry(frmTus, justify=CENTER, bd=3, width=15)
        ent2.insert(0, self.ara_not2)
        lab.grid(row=row, sticky=W, padx=15)
        ent2.grid(row = row+1, sticky=W, padx=15)
        self.ent_ara_not2 = ent2


        row += 3
        lab = Label(frmTus, text='Genel Sınav Notu', width=15)
        ent3 = Entry(frmTus, justify=CENTER, bd=3, width=15)
        ent3.insert(0, self.genel_not)
        lab.grid(row=row, sticky=W, padx=15)
        ent3.grid(row = row+1, sticky=W, padx=15)
        self.ent_genel_not = ent3
